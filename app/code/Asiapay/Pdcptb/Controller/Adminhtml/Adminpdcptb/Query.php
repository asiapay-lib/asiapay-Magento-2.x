...
class Query extends AbstractAdminpdcptb
{
    /**
    	...
    	if($response == ''){
    			...
    			echo 'QUERY RESPONSE: No response recieved.<br/><br/>';
    				...
    				if($hasAtleastOneApproved){
    					echo '<br/>Are you sure you want to <b>update the state of the order to Processing</b>? <br/><br/> <a href="' . $this->getUrl('pdcptb/adminpdcptb/update/',['order_id'=>$order_object->getId()]) . '">[ Update State ]</a> OR ';
    				}else{
    					echo '<br/>Are you sure you want to <b>cancel the order</b>? <br/><br/> <a href="' . $this->getUrl('pdcptb/adminpdcptb/cancel/',['order_id'=>$order_object->getId()]) . '">[ Cancel Order ]</a> OR ';
    				}
